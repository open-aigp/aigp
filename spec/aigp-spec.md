...
**Latest published version:** https://github.com/open-aigp/aigp
...
- [12. Conformance Levels](#12-conformance-levels)
  - [12.1 Core Conformance](#121-core-conformance)
  - [12.2 Extended Conformance](#122-extended-conformance)
  - [12.3 Full Conformance](#123-full-conformance)
  - [12.4 Conformance Profiles](#124-conformance-profiles)
  - [12.5 Reference Sector Profiles](#125-reference-sector-profiles)
- [13. Transport Bindings via CloudEvents](#13-transport-bindings-via-cloudevents)
  - [13.1 CloudEvents Context Attribute Mapping](#131-cloudevents-context-attribute-mapping)
  - [13.2 AIGP Extension Attributes](#132-aigp-extension-attributes)
  - [13.3 Content Modes](#133-content-modes)
  - [13.4 Batch Delivery](#134-batch-delivery)
  - [13.5 Transport Security](#135-transport-security)
  - [13.6 Receiver Behavior](#136-receiver-behavior)
...
- [Appendix D: Reference Audit Viewer Requirements](#appendix-d-reference-audit-viewer-requirements)
...
| `sequence_number` | Integer | Monotonically increasing counter scoped to (`agent_id`, `trace_id`). MUST start at `1` and increase by exactly `1` for subsequent events in the same scope. |
...
- **Notes:** This event provides visibility into trust boundary crossings during partial AIGP adoption. Governed agents record their side of the interaction; the ungoverned system's behavior is unattested. When exchanged content affects downstream decisions, implementations SHOULD hash a normalized boundary claim (request payload + response payload + target identifier) and place that hash in `governance_hash`. Implementations SHOULD use `causality_ref` to link this event to the preceding governance action.
...
- `content_ref` — Required when `hash_mode` is `"pointer"`.
- `is_salted` — `true` when a producer-side salt/nonce participated in hash derivation.
- `salt_ref` — Required when `is_salted` is `true`; opaque locator for verifier retrieval of salt/nonce material.
- `is_partial` — `true` when hash represents partial stream output.
- `offset_unit` and `offset` — Required when `is_partial` is `true`; define where interruption occurred.

#### 8.8.6 Inclusion Proof Structure

When `inclusion_proofs` is present, each proof object MUST contain:

- `leaf_hash` — The 64-character lowercase hexadecimal hash of the leaf being proven.
- `proof_path` — Ordered array of sibling steps from leaf level to root. Each step MUST contain:
  - `sibling_hash` — The sibling node hash at that level (64-character lowercase hex).
  - `sibling_position` — Either `"left"` or `"right"`, indicating where `sibling_hash` was positioned relative to the current node during parent computation.

Example:

```json
{
  "leaf_hash": "1138149eb8ee4355ad80ee3c02a93c46a2b82c8976eaa9ba7c304b740cc6921a",
  "proof_path": [
    {
      "sibling_hash": "16ec2355054a1f60f476c20f54ccfa6056b0b224a6582a19307a3b9eabc8bfec",
      "sibling_position": "right"
    },
    {
      "sibling_hash": "5769525038c226978cb78b401f0c62c00442b5b6c48f0a9cb9a7cff504f75366",
...
A verifier who possesses only a subset of resources can verify leaf membership using `inclusion_proofs` without reconstructing the full tree:

1. Start with `current = leaf_hash`.
2. For each `proof_path` step in order:
   - If `sibling_position` is `"left"`, compute `current = SHA-256(UTF-8(sibling_hash + current))`.
...
#### 8.8.9 Salted Leaf Hashing (Privacy-Focused Optional Extension)

For privacy-sensitive governed resources (for example, memory content containing personal data), implementations MAY use salted leaf hashing.

When salted hashing is used for a leaf:

- `is_salted` MUST be `true`.
- `salt_ref` MUST be present and non-empty.
- Raw salt/nonce bytes MUST NOT be embedded in the AIGP event.

Recommended derivation:

```
leaf_hash = SHA-256(UTF-8(resource_type + ":" + resource_name + ":" + salt + ":" + payload))
```

Where:

- `payload` is either `content` (content mode) or `content_ref` (pointer mode).
- `salt` is retrieved out-of-band by authorized verifiers using `salt_ref`.

`salt_ref` is an opaque locator (for example, KMS/HSM/vault URI). AIGP does not mandate a specific key management system.

#### 8.8.10 Stream Interruption Metadata (Partial Output Proofs)

When an event captures partial generated content (for example, `INFERENCE_BLOCKED` or `POLICY_VIOLATION` during streaming), implementations SHOULD represent that partial content as a governed leaf and set:

- `is_partial` = `true`
- `offset_unit` = one of `token`, `byte`, or `char`
- `offset` = zero-based interruption position in `offset_unit`

This allows a verifier to reproduce the exact partial payload hash and prove that interruption happened at a specific stream boundary.
...
1. Every produced AIGP event MUST contain all required fields as defined in Section 5.1 (`event_id`, `event_type`, `event_category`, `event_time`, `agent_id`, `governance_hash`, `trace_id`, `sequence_number`).
...
Organizations MAY define named conformance profiles layered on top of Core/Extended/Full. A profile narrows optionality for a specific regulatory or operational context.

Profile requirements:

1. A profile definition MUST declare its base conformance level (`core`, `extended`, or `full`).
2. A profile MUST define any additional required event types, field requirements, or validation checks.
3. A profile MUST NOT relax or override any mandatory requirement from its declared base level.
4. Profile conformance claims SHOULD be machine-readable and auditable (for example, by publishing profile metadata in deployment documentation or registry configuration).

For profiles that require signed events, the following rule applies:

- Any event with `severity` set to `"high"` or `"critical"` MUST include non-empty `event_signature` and `signature_key_id`.
- The signature MUST verify against a trusted key for the declared producer identity.

For profiles that require causal ordering checks, the following rules apply:

- Any event with a non-empty `trace_id` MUST include `sequence_number` >= 1.
- Within each (`agent_id`, `trace_id`) scope, `sequence_number` MUST start at 1 and increase by exactly 1 for each subsequent event.
- If `causality_ref` is non-empty, it MUST reference an existing `event_id` in the same audit evidence set.
- Verifiers SHOULD report ordering findings using stable identifiers: `SEQUENCE_GAP`, `SEQUENCE_DUPLICATE`, `SEQUENCE_RESET`, `BROKEN_CAUSAL_REF`.

For profiles that require cryptographic integrity checks, verifiers SHOULD report findings using stable identifiers:

- `SIGNATURE_VERIFICATION_FAILED`
- `MERKLE_ROOT_MISMATCH`
- `INCLUSION_PROOF_INVALID`

### 12.5 Reference Sector Profiles

This specification defines two reference profiles to reduce adoption burden for smaller teams. These profiles are normative templates; organizations MAY extend them.

#### 12.5.1 HIPAA Profile (Reference)

Base level: **Full**

Additional requirements:

1. Implementations MUST emit `INFERENCE_STARTED`, `INFERENCE_COMPLETED`, and `HUMAN_APPROVAL` for automated clinical decision workflows.
2. Implementations MUST emit `UNVERIFIED_BOUNDARY` whenever governed workflows call non-AIGP external clinical systems.
3. Events with `severity` `"high"` or `"critical"` MUST be signed per Section 12.4.
4. `data_classification` MUST be populated for all events in patient-related traces.
5. Events in patient-related traces MUST satisfy profile-level causal ordering checks per Section 12.4 (`sequence_number` monotonic checks, plus `causality_ref` validation when populated).

#### 12.5.2 FINRA Profile (Reference)

Base level: **Full**

Additional requirements:

1. Implementations MUST emit `PROMPT_USED`, `TOOL_INVOKED`, and `POLICY_VIOLATION` for governed trading and research workflows.
2. Implementations MUST emit `UNVERIFIED_BOUNDARY` for any external venue, market data, or counterparty system that does not provide verifiable AIGP evidence.
3. Events with `severity` `"high"` or `"critical"` MUST be signed per Section 12.4.
4. `data_classification` MUST be populated for all events involving confidential or restricted financial data.
5. Events in governed trading and research traces MUST satisfy profile-level causal ordering checks per Section 12.4 (`sequence_number` monotonic checks, plus `causality_ref` validation when populated).

---

## 13. Transport Bindings via CloudEvents

AIGP events are transported using the [CloudEvents](https://cloudevents.io/) specification (CNCF, v1.0). CloudEvents provides a vendor-neutral envelope with protocol bindings for HTTP, Kafka, AMQP, NATS, gRPC, and WebSockets. By adopting CloudEvents as the transport layer, AIGP gains interoperability with every CloudEvents-compatible broker, gateway, and serverless platform without defining custom transport bindings.

The AIGP event is the CloudEvents `data` payload. CloudEvents context attributes provide routing and filtering metadata. AIGP extension attributes (Section 13.2) surface key governance fields at the envelope level for broker-level filtering without payload deserialization.

### 13.1 CloudEvents Context Attribute Mapping

Every AIGP event transmitted via CloudEvents MUST include the following context attributes:

| CloudEvents Attribute | Required | AIGP Mapping | Description |
|---|---|---|---|
| `specversion` | REQUIRED | `"1.0"` | CloudEvents spec version. |
| `id` | REQUIRED | `event_id` | AIGP event UUID. MUST match the `event_id` field in the data payload. |
| `type` | REQUIRED | `"org.aigp.v1."` + lowercase `event_type` | Reverse-DNS event type. Example: `"org.aigp.v1.inject_success"`. See Section 13.1.1. |
...
The CloudEvents `type` attribute MUST follow the pattern:

```
org.aigp.v1.<lowercase_event_type>
```

Where `<lowercase_event_type>` is the AIGP `event_type` converted to lowercase. Examples:

| AIGP `event_type` | CloudEvents `type` |
|---|---|
| `INJECT_SUCCESS` | `org.aigp.v1.inject_success` |
| `GOVERNANCE_PROOF` | `org.aigp.v1.governance_proof` |
| `MEMORY_READ` | `org.aigp.v1.memory_read` |
| `UNVERIFIED_BOUNDARY` | `org.aigp.v1.unverified_boundary` |

The `v1` segment enables future major version evolution. Consumers MAY use prefix matching (`org.aigp.v1.*`) to subscribe to all AIGP v1 events, or filter by category (e.g., `org.aigp.v1.inject_*`).

### 13.2 AIGP Extension Attributes

AIGP defines the following CloudEvents [extension attributes](https://github.com/cloudevents/spec/blob/main/cloudevents/spec.md#extension-context-attributes) for envelope-level routing and filtering. Extension attribute names conform to CloudEvents naming rules: lowercase `[a-z0-9]` only, max 20 characters.

| Extension Attribute | Type | AIGP Mapping | Description |
|---|---|---|---|
| `aigpagentid` | String | `agent_id` | AGRN agent identifier. Enables per-agent routing. |
| `aigporgid` | String | `org_id` | AGRN organization identifier. Enables per-org routing. |
| `aigpcategory` | String | `event_category` | Event category (e.g., `"inject"`, `"memory"`, `"inference"`). |
| `aigpclassification` | String | `data_classification` | Data sensitivity tier (`"public"`, `"internal"`, `"confidential"`, `"restricted"`). Enables routing restricted events to compliance-only queues. |
| `aigpseverity` | String | `severity` | Impact level (`"critical"`, `"high"`, `"medium"`, `"low"`). Enables priority-based routing. |
| `aigphashtype` | String | `hash_type` | Hash algorithm (`"sha256"`, `"merkle-sha256"`). |

All AIGP extension attributes are OPTIONAL. Producers SHOULD include `aigpagentid` and `aigpcategory` at minimum to enable meaningful broker-level filtering.

### 13.3 Content Modes

AIGP supports both CloudEvents content modes. Implementations MUST support structured mode. Binary mode is RECOMMENDED for high-throughput transports.

#### 13.3.1 Structured Content Mode

The entire CloudEvents envelope (context attributes + AIGP event as `data`) is serialized as a single JSON document. Used for event brokers, serverless platforms, and any system that inspects events as JSON.
...
```json
{
  ...
  "data": {
    ...
    "event_time": "2026-02-15T14:30:00.123Z",
...
HTTP `Content-Type`: `application/cloudevents+json; charset=utf-8`

#### 13.3.2 Binary Content Mode

CloudEvents context attributes are transported as protocol headers. The HTTP body or Kafka message value contains the raw AIGP event JSON. No `data` wrapper — the AIGP event IS the payload.

**HTTP example:**

```
POST /v1/aigp HTTP/1.1
Content-Type: application/json
ce-specversion: 1.0
ce-id: a1b2c3d4-e5f6-7890-abcd-ef1234567890
ce-type: org.aigp.v1.inject_success
ce-source: aigp://org.finco/agent.trading-bot-v2
ce-time: 2026-02-15T14:30:00.123Z
ce-aigpagentid: agent.trading-bot-v2
ce-aigporgid: org.finco
ce-aigpcategory: inject
ce-aigpclassification: confidential

{"event_id":"a1b2c3d4-e5f6-7890-abcd-ef1234567890","event_type":"INJECT_SUCCESS",...}
```

**Kafka example:**

Headers (note `ce_` underscore prefix per CloudEvents Kafka binding):
```
ce_specversion: 1.0
ce_id: a1b2c3d4-e5f6-7890-abcd-ef1234567890
ce_type: org.aigp.v1.inject_success
ce_source: aigp://org.finco/agent.trading-bot-v2
ce_aigpagentid: agent.trading-bot-v2
ce_aigpcategory: inject
```

Message key: `agent.trading-bot-v2` (enables partitioning by agent)

Message value: raw AIGP event JSON

### 13.4 Batch Delivery

Multiple AIGP events MAY be delivered in a single CloudEvents batch.

- HTTP: `Content-Type: application/cloudevents-batch+json; charset=utf-8`. Body is a JSON array of CloudEvents-wrapped AIGP events (structured mode).
- Kafka: Each Kafka message carries a single CloudEvents-wrapped AIGP event. Batching is handled at the Kafka producer level (not at the CloudEvents level).

All events in a batch MUST share the same `specversion` value.

### 13.5 Transport Security

- AIGP events MUST be transported over TLS 1.2 or later (see Section 14.3).
- Events with `aigpclassification` of `"confidential"` or `"restricted"` MUST NOT be transmitted over unencrypted channels.

### 13.6 Receiver Behavior

- Receivers SHOULD return HTTP 202 (Accepted) upon successful receipt.
- Receivers MUST validate the `specversion` and `type` attributes before processing.
- Receivers MAY use the `aigpcategory` and `aigpclassification` extension attributes for routing decisions without deserializing the `data` payload.
- Receivers that do not recognize an AIGP extension attribute MUST ignore it (per CloudEvents extension semantics).
...
Conformance profiles MAY require signing for specific event classes (for example, `severity` `"high"` or `"critical"` events in regulated profiles; see Section 12.4).
...
## Appendix D: Reference Audit Viewer Requirements

This appendix defines minimum requirements for a reference audit viewer implementation. The goal is to make AIGP verification usable by non-technical auditors while preserving cryptographic rigor.

An audit viewer implementation SHOULD provide:

1. **Causal DAG visualization** — render event dependencies using `sequence_number` and `causality_ref`, with cross-agent edges clearly marked.
2. **Signature verification status** — verify `event_signature` against trusted keys and display pass/fail state per event, including key identifier (`signature_key_id`).
3. **Merkle verification workflows** — support both full-tree verification and selective inclusion-proof verification using `governance_merkle_tree.inclusion_proofs`.
4. **Boundary visibility** — visually highlight `UNVERIFIED_BOUNDARY` events and display associated boundary claim hash inputs where present.
5. **Profile checks** — evaluate profile-level rules (Section 12.4 and Section 12.5), including mandatory signatures for high/critical severity events.

An audit viewer SHOULD export a machine-readable verification report that includes:

- Verification timestamp.
- Event-level pass/fail results.
- Signature verification outcomes.
- Merkle proof verification outcomes.
- Profile rule violations, if any.

Implementations SHOULD emit finding identifiers from the following baseline taxonomy:
...
The recommended machine-readable report contract is published at:

- `schema/aigp-verifier-report.schema.json`

This appendix is implementation guidance, not a transport or UI standard. Different products MAY choose different visualization styles while preserving these minimum verification capabilities.
...
- **[AgentGP]** AgentGP -- First reference implementation of the AIGP specification. https://github.com/agentgp-io/agentgp
