...
EMPTY_HASH_EVENT_TYPES = {"AGENT_REGISTERED", "AGENT_APPROVED", "AGENT_DEACTIVATED"}
...
def _validate_event(event: dict) -> list[str]:
    ...
    elif not governance_hash and event.get("event_type") not in EMPTY_HASH_EVENT_TYPES:
...
def test_conformance_fixtures():
    fixture_path = pathlib.Path(__file__).resolve().parents[3] / "conformance" / "validation-fixtures.tsv"
