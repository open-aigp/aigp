...
- Repository topology split into dedicated repos:
  ...
  - `open-aigp/aigp-tools` (verifier CLI and audit UI tooling)

## [0.11] - 2026-02-20

### Added
- Optional Merkle leaf privacy fields: `is_salted`, `salt_ref`
- Optional Merkle leaf stream-interruption fields: `is_partial`, `offset_unit`, `offset`
- Verifier report schema: `schema/aigp-verifier-report.schema.json`
- Stable verifier integrity finding IDs:
  - `SIGNATURE_VERIFICATION_FAILED`
  - `MERKLE_ROOT_MISMATCH`
  - `INCLUSION_PROOF_INVALID`

### Changed
- Specification version advanced to `0.11` (minor SemVer line for spec + schemas)
- Governance versioning guidance updated to minor SemVer stream
- Conformance fixture guidance aligned for empty `governance_hash` only on no-content events

## [0.10.0] - 2026-02-19

### Added
- **Merkle inclusion proofs** for selective verification in `governance_merkle_tree.inclusion_proofs`
- Protobuf types for proof paths: `SiblingPosition`, `MerkleProofStep`, `MerkleInclusionProof`
- JSON Schema support for `inclusion_proofs` with ordered `proof_path` validation (`sibling_hash`, `sibling_position`)
- SDK parity (Python, TypeScript, Go, Rust, Java, Kotlin, .NET): inclusion proof helpers with spec-aligned `leaf_hash` + `proof_path`
- SDK parity (Python, TypeScript, Go, Rust, Java, Kotlin, .NET): vendor-neutral signer interfaces (`EventSigner`/`IEventSigner`) and sign helpers
- SDK parity (Python, TypeScript, Go, Rust, Java, Kotlin, .NET): transport-agnostic delivery helpers (`RetryPolicy`, `ReliableEmitter`)
- Python SDK reference signer implementation: `ES256PrivateKeySigner`
- Spec Section 12.4: conformance profiles layered on Core/Extended/Full
- Spec Section 12.5: reference HIPAA and FINRA sector profiles
- Appendix D: reference audit viewer requirements (DAG, signature checks, Merkle proof checks, profile checks)

### Changed
- Spec version advanced to 0.10.0 draft
- `UNVERIFIED_BOUNDARY` guidance now recommends hashing normalized boundary claims when exchanged external content influences downstream decisions
- Merkle verification section now defines root-verifiable subset checks using inclusion proofs

## [0.9.0] - 2026-02-15

### Added
- **Normative Protobuf schema** (`schema/aigp-event.proto`) — single source of truth for AIGP event types, enabling multi-language SDK code generation via `buf generate`
- Proto3 enums: `DataClassification`, `Severity`, `HashType`, `HashMode` — type-safe representations of AIGP classification tiers
- Proto3 messages: `AIGPEvent`, `GovernanceMerkleTree`, `MerkleLeaf`, `AIGPEventBatch` — full event structure with field number stability for wire compatibility
- `google.protobuf.Timestamp` for `event_time` and `ingested_at` — native RFC 3339 JSON mapping
- `google.protobuf.Struct` for `annotations` — preserves arbitrary key-value semantics
- Buf configuration (`schema/buf.yaml`, `schema/buf.gen.yaml`) — linting, breaking change detection, and code generation targets for TypeScript, Go, and Python
- Language-specific package options: `go_package`, `java_package`, `csharp_namespace` — ready for multi-language SDK scaffolding
- **CloudEvents transport binding** — AIGP events transported via CloudEvents (CNCF Graduated, v1.0), replacing the placeholder Section 13 with a full normative binding
- Spec Section 13 rewritten: CloudEvents context attribute mapping, AIGP extension attributes, structured and binary content modes, batch delivery, receiver behavior
- **6 AIGP extension attributes** for envelope-level routing: `aigpagentid`, `aigporgid`, `aigpcategory`, `aigpclassification`, `aigpseverity`, `aigphashtype` — all lowercase `[a-z0-9]` per CloudEvents naming rules
- **Type convention**: `org.aigp.v1.<lowercase_event_type>` (reverse-DNS, versioned) — enables prefix-based subscription filtering
- **Source convention**: `aigp://<org_id>/<agent_id>` URI scheme
- Integration directory: `integrations/cloudevents/` with README, structured-mode examples (inject-success, governance-proof)
- Python SDK: `cloudevents` module with `wrap_as_cloudevent()`, `unwrap_from_cloudevent()`, `build_ce_headers()`, `ce_type_from_event_type()`, `event_type_from_ce_type()`
- 38 new CloudEvents tests (total: 209)

### Changed
- Proto is now the normative type definition; JSON Schema (`aigp-event.schema.json`) remains as a conformance/validation artifact
- Spec Section 13 title: "Transport Bindings" → "Transport Bindings via CloudEvents"
- HTTP, Kafka, gRPC transport conventions now specified through CloudEvents protocol bindings rather than AIGP-specific rules
- All version references bumped to 0.9.0
- Backward compatible: no wire format changes, existing JSON events are valid v0.9.0 events
...
- Python SDK (`sdks/python/aigp`) — AIGP-OTel bridge library with:
...
- Schema `$id` updated to `https://aigp.ui.sandarb.ai/schema/aigp-event.schema.json`
