...
Attaches to `inputs[].inputFacets.aigp_resource`. Models each governed resource (policy, prompt, tool, context, lineage, memory, model) as an OpenLineage InputDataset.

Schema: [`facets/AIGPResourceInputFacet.json`](facets/AIGPResourceInputFacet.json)

## Python SDK

The `aigp.openlineage` module builds OpenLineage-compatible dicts with zero OpenLineage library dependency:
